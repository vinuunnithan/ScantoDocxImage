...
if not is_signed:
    ...
    def create_all_files_zip_archive(clean_name, docx_bytes, markdown_text, zip_bytes=None, extracted_figures=None):
        """
        Creates a master in-memory zip archive bundling all generated outputs:
        - {clean_name}.docx (Compiled Word document)
        - {clean_name}.md (Optimized Markdown source)
        - {clean_name}_images.zip (Standalone archive of all extracted images, if any)
        - images/ (Direct directory with all extracted images for immediate local markdown preview)
        - archive_contents.txt (Summary of files included in the package)
        """
        master_buffer = io.BytesIO()
        with zipfile.ZipFile(master_buffer, "w", zipfile.ZIP_DEFLATED) as master_zip:
            # 1. Add Word document (.docx)
            master_zip.writestr(f"{clean_name}.docx", docx_bytes)

            # 2. Add Markdown source (.md)
            master_zip.writestr(f"{clean_name}.md", markdown_text.encode("utf-8"))

            # 3. Add Images ZIP archive (.zip) if any figures were extracted
            if zip_bytes:
                master_zip.writestr(f"{clean_name}_images.zip", zip_bytes)

            # 4. Also include individual extracted images in an 'images/' folder for instant local Markdown viewing
            if extracted_figures:
                for fig in extracted_figures:
                    fig_path = fig.get("path")
                    if fig_path and os.path.exists(fig_path):
                        with open(fig_path, "rb") as f_img:
                            master_zip.writestr(f"images/{fig['id']}", f_img.read())

            # 5. Add manifest file
            manifest_lines = [
                f"Master Conversion Archive for: {clean_name}",
                f"Generated by: PNG/PDF to Markdown & Word Converter (Gemini AI)",
                "=" * 60,
                f"1. {clean_name}.docx : Formatted Microsoft Word document.",
                f"2. {clean_name}.md   : Transcribed Markdown source with equations & figure tags.",
            ]
            if zip_bytes:
                manifest_lines.append(f"3. {clean_name}_images.zip : Standalone zip archive of extracted figures ({len(extracted_figures)} image(s)).")
                manifest_lines.append(f"4. images/ : Folder containing extracted images for local Markdown viewing.")
            manifest_lines.append("=" * 60)

            master_zip.writestr("archive_contents.txt", "\n".join(manifest_lines).encode("utf-8"))
...
"""
                    ...
                    with tempfile.TemporaryDirectory() as tmpdir:
                        ...
                        clean_name = output_name.strip().replace(".docx", "").replace(".md", "").replace(".zip", "") or "converted_document"
                        ...
                        # Create master ZIP archive bundling all files (docx, md, and zip)
                        all_files_zip_bytes = create_all_files_zip_archive(
                            clean_name=clean_name,
                            docx_bytes=docx_bytes,
                            markdown_text=optimized_markdown,
                            zip_bytes=zip_bytes,
                    ...
                    if zip_bytes:
                        ...
                        with col4:
                            st.download_button(
                                ...
                                data=all_files_zip_bytes,
                                file_name=f"{clean_name}_all_files.zip",
                                mime="application/zip",
                                use_container_width=True
                            )
                    else:
